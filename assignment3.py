...
class Assignment3:
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        intersection_points = np.sort(Assignment2.intersections(Assignment2(), f1, f2, 0, 100))
        i = 1
        result = 0
        ...
        while i < len(intersection_points):
            first_point = intersection_points[i-1]
            second_point = intersection_points[i]
            result = result + np.abs(self.integrate(f, first_point, second_point, 100))
            i = i + 1
